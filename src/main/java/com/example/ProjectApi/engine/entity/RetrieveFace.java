package com.example.ProjectApi.engine.entity;
...
public class RetrieveFace extends EngineFunc {
	...
	public void generateCli() {
		...
		if (attributeCheck.stringsNotNull(ENGINEPATH, disk, trainedBinaryPath, trainedFaceInfoPath, JsonPath)) {
			...
			cli = new StringBuilder("cd " + ENGINEPATH + " && " + disk + ": && RetrieveFace " + "--threshold "
					+ threshold + " " + (isHideMainWindow == true ? " --hide-main-window" : "") + "--show-thread-window"
					+ "--resolution " + resolution + " " + "--output-frame " + outputFramePath + " " + "--output-face "
					+ outputFacePath + " " + inputSource + " " + "--minimum-face-size " + minimumFaceSize + " "
					+ "--threads " + threads + " " + trainedBinaryPath + " " + trainedFaceInfoPath + " " + JsonPath
					+ "");
		} else {
			cli = null;
		}
